...
import gitlab
from tools import reset_gitlab


CONFIG_FILE = "/tmp/python-gitlab.cfg"
LOGIN = "root"
PASSWORD = "5iveL!fe"
...
def gl():
    ...
    # Teardown for flaky re-runs
    reset_gitlab.main()


def test_token_auth_from_config_file():
    gl = gitlab.Gitlab.from_config(config_files=[CONFIG_FILE])
    gl.auth()
...
def test_api_v4(gl):
    ...
    badge_image = "http://example.com"
    badge_link = "http://example/img.svg"
    badge = admin_project.badges.create(
